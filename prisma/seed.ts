...
async function main() {
  ...
  await db.pilotLead.deleteMany({});
  await db.pilotLead.createMany({
    data: [
      {
        kind: "INTERVIEW",
        status: "INTERVIEWED",
        persona: "EA / chief of staff",
        name: "Alex Rivera",
        email: "alex.rivera@example.com",
        organisation: "Mayfair FO desk",
        city: "London",
        categoriesJson: JSON.stringify(["personal-assistant", "chauffeur"]),
        interestScore: 8,
        nextUseCase: "Investor week PA + airport transfers",
        wtpNotes: "Open to take-rate if vetting depth proven",
        painsJson: JSON.stringify([
          "Ad-hoc referrals fail discretion",
          "Agency slow on emergency cover",
        ]),
        trustIncident: true,
        pilotWilling: true,
        warmIntros: 2,
        source: "ops",
        interviewedAt: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000),
        createdById: ops.id,
      },
      {
        kind: "INTERVIEW",
        status: "PILOT_YES",
        persona: "Boutique agency",
        name: "Priya Shah",
        email: "priya@mayfairstaff.example",
        organisation: "Mayfair Staffing Partners",
        city: "London",
        categoriesJson: JSON.stringify(["personal-assistant"]),
        interestScore: 9,
        nextUseCase: "White-label trust layer for 3 FO clients",
        wtpNotes: "Placement fee share preferred",
        painsJson: JSON.stringify(["Cannot productise own vetting"]),
        trustIncident: false,
        pilotWilling: true,
        warmIntros: 3,
        source: "ops",
        interviewedAt: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000),
        createdById: ops.id,
      },
      {
        kind: "INTERVIEW",
        status: "INTERVIEWED",
        persona: "Family office / household",
        name: "Jordan Lee",
        email: "j.lee@example.com",
        organisation: "Private household",
        city: "London",
        categoriesJson: JSON.stringify(["chauffeur", "household-manager"]),
        interestScore: 6,
        nextUseCase: null,
        wtpNotes: "Price sensitive vs existing driver",
        painsJson: JSON.stringify(["Already has trusted driver"]),
        trustIncident: false,
        pilotWilling: false,
        warmIntros: 0,
        source: "ops",
      ...
      },
      {
        ...
        organisation: "Tech founder EA",
        city: "London",
        categoriesJson: JSON.stringify(["personal-assistant"]),
        nextUseCase: "Travel PA Q3",
        source: "landing",
      },
      {
        kind: "SUPPLY",
        ...
        city: "London",
        categoriesJson: JSON.stringify(["personal-assistant"]),
        notes: "Ex-chief of staff seeking platform placement",
        source: "landing",
      },
    ],
  });
